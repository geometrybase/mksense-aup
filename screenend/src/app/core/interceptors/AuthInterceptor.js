...
(function() {
  ...
  angular.module('mksense.core.interceptors')
    .factory('AuthInterceptor', [
      '$q', '$injector', '$localStorage','$stateParams',
      function(
        $q, $injector, $localStorage,$stateParams
      ) {
        return {
          ...
          request: function requestCallback(config) {
            ...
            if ($localStorage.credentials && $localStorage.credentials[$stateParams.exhibitionId]) {
              token = $localStorage.credentials[$stateParams.exhibitionId].token;
            }
          ...
          responseError: function responseErrorCallback(response) {
            if (response.status === 401) {
              $localStorage.$reset();
              $injector.get('$state').go('exhibition.passcode',$injector.get('$state').current.params);
            }
