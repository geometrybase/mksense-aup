(function() {
  ...
  .factory('ScreenService',[
    ...
    function(
      ...
      var logger=$log.getInstance('SCREEN-SERVICE');

      function ScreenService(screenConfig){
        EventEmitter.call(this);
        var self=this;
        self.exhibitionId=screenConfig.exhibitionId;
        self.sceneId=screenConfig.sceneId;
        self.screenIndex=screenConfig.screenIndex;
        self.isMaster=self.screenIndex===0;
        self.connectionsMap={};
        ...
        //$http.get(mkConfig.cmsServer+screenConfig.organizationName+'/screen/'+screenConfig.screenName,{
          //cache:true
        //})
        //.success(function(data, status, headers, config) {
          //if(!data || !data['visuals'] || !data['timeline'] || !data['screens'] || !data['screens'][screenConfig.screenIndex]){
            //logger.debug(data.visuals);
            //return self.emit('error','Incomplete screen response from '+mkConfig.cmsServer+screenConfig.organizationName+'/'+screenConfig.screenName);
          //}else{
            //logger.debug('screen data',data.visuals);
            //self.screens=data.screens; 
            //self.visuals=data.visuals;
            //self.timeline=data.timeline;
            //self.screenDescription=data.description;
        ...
        function registerSocketEvents(){
          ...
          self.socket.on('joinScreenDone',function(rooms){
            logger.debug(rooms);
            self.rooms=rooms;
      ...
      ScreenService.prototype=_.create(EventEmitter.prototype,{
        'constructor':ScreenService
      });

      ScreenService.prototype.nextVisual=function(){
        var self=this;
      ...
      ScreenService.prototype.fadeOut=function(time){
      ...
      ScreenService.prototype.delay=function(delay){
        var self=this; 
        self.fadeOutInterval+=delay;
        self.fadeOut(self.fadeInTime+self.fadeOutInterval-Date.now());
      }

      ScreenService.prototype.publishToScreens=function(data){
        var self=this;
        if(!self.isMaster)
          return 
      ...
      ScreenService.prototype.sendToMaster=function(data){
        var self=this;
      ...
      ScreenService.prototype.sendData=function(peerId,data){
        var self=this;
        if(self.connectionsMap[peerId]) 
          self.connectionsMap[peerId].send(data);
      };

      ScreenService.prototype.getCurrentVisual=function(){
        return this.visuals[this.visualIndex]; 
      };
      ScreenService.prototype.getCurrentVisualType=function(){
        return this.visuals[this.visualIndex].type; 
      };

      ScreenService.prototype.getNextVisual=function(){
      ...
      ScreenService.prototype.destroy=function(){
